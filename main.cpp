...
// instantiate some number of boids
  // set up the behaviors: avoid, cohesion, gather
  // each boid will be a struct probably, with a position vector, velocity vector, force accumulator vector
  // eventually we will want an obj file for the mesh, doesn't need to be a part of the struct
// will need to use the instanced renderable thing to create lots of the same object
  // in the simulation loop, as we determine the transformation matrices for each of the boids
  // add the boid to the instanced list
  // then do a single draw call once all of them are added
...
// Forces can be modified with the curves panel thing

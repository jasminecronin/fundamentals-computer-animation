...
inline void spline(const float *key, int num, int dim, float t, float *v) {
  static signed char coefs[16] = {-1, 2, -1, 0, 3, -5, 0, 2,
                                  -3, 4, 1,  0, 1, -1, 0, 0};
  ...
  // interpolant
  const float h =
      (t - key[(k - 1) * size]) / (key[k * size] - key[(k - 1) * size]);
...
inline int Curve(const char *label, const ImVec2 &size, int &item,
  ...
  const bool hovered = IsItemHovered(); // IsItemHovered(bb, id);
  ...
  // bg grid
  window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Min.y + ht / 2),
                            ImVec2(bb.Max.x, bb.Min.y + ht / 2),
                            GetColorU32(ImGuiCol_TextDisabled), 3);

  window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Min.y + ht / 4),
                            ImVec2(bb.Max.x, bb.Min.y + ht / 4),
                            GetColorU32(ImGuiCol_TextDisabled));

  window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Min.y + ht / 4 * 3),
  ...
  for (i = 0; i <= (smoothness - 1); ++i) {
    ...
    ImVec2 p(px * (bb.Max.x - bb.Min.x) + bb.Min.x,
             py * (bb.Max.y - bb.Min.y) + bb.Min.y);
    ImVec2 q(qx * (bb.Max.x - bb.Min.x) + bb.Min.x,
             qy * (bb.Max.y - bb.Min.y) + bb.Min.y);
